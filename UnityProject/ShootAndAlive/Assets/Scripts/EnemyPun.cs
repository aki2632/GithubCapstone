...
// 적 AI를 구현한다
public class EnemyPun : LivingEntityPun
{
    public LayerMask whatIsTarget; // 공격 대상 레이어

    private LivingEntityPun targetEntityPun; // 추적할 대상
    ...
    public override void Die()
    {
        ...
        // 플레이어 경험치 증가
        GameManagerPun.instance.GetExp();
    }
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        if (!dead && Time.time >= lastAttackTime + timeBetAttack)
        {
            ...
            LivingEntityPun attackTarget
                = other.GetComponent<LivingEntityPun>();

            // 상대방의 LivingEntity가 자신의 추적 대상이라면 공격 실행
            if (attackTarget != null && attackTarget == targetEntityPun)
            {
                // 최근 공격 시간을 갱신
