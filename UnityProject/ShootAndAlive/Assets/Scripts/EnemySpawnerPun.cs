...
// 적 게임 오브젝트를 주기적으로 생성
public class EnemySpawnerPun : MonoBehaviourPun, IPunObservable {
    public EnemyPun enemyPrefabPun; // 생성할 적 AI
    ...
    private List<EnemyPun> enemies = new List<EnemyPun>(); // 생성된 적들을 담는 리스트
    ...
    private void Update() {
        ...
        {
            // 게임 오버 상태일때는 생성하지 않음
            if (GameManagerPun.instance != null && GameManagerPun.instance.isGameover)
            {
                return;
            }
    ...
    private void CreateEnemy(float intensity) {
        ...
        GameObject createdEnemy = PhotonNetwork.Instantiate(enemyPrefabPun.gameObject.name,
            spawnPoint.position,
            spawnPoint.rotation);
        
        // 생성한 적을 셋업하기 위해 Enemy 컴포넌트를 가져옴
        EnemyPun enemyPun = createdEnemy.GetComponent<EnemyPun>();

        // 생성한 적의 능력치와 추적 대상 설정
        enemyPun.photonView.RPC("Setup", RpcTarget.All, health, damage, speed,
            skinColor);

        // 생성된 적을 리스트에 추가
        enemies.Add(enemyPun);

        // 적의 onDeath 이벤트에 익명 메서드 등록
        // 사망한 적을 리스트에서 제거
        enemyPun.onDeath += () => enemies.Remove(enemyPun);
        // 사망한 적을 10 초 뒤에 파괴
        enemyPun.onDeath += () => StartCoroutine(DestroyAfter(enemyPun.gameObject, 10f));
        // 적 사망시 점수 상승
        enemyPun.onDeath += () => GameManagerPun.instance.AddScore(100);
        }
